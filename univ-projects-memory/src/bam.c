...
Memseg_t* get_memseg() {
	...
	for(i = 0; i < LINKTAB_SIZE; i++) {
		if (!t[i].next && !t[i].seg_start && !t[i].seg_size) {
			return &t[i];
		}
	}
...
void* bam_realloc(void* ptr, size_t size, char* filename, unsigned line) {
	...
	if (!ulist) {
		ip_error_display(ptr, "bam_realloc", filename, line, 0);
	}
...
void bam_free(void* ptr, char* filename, unsigned line) {
	...
	if (!ulist) {
		ip_error_display(ptr, "bam_free", filename, line, 1);
	}
