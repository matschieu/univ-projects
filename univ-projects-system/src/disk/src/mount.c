/* ------------------------------
   $Id: mount.c,v 1.1 2009/11/16 05:38:07 marquet Exp $
   ------------------------------------------------------------
...
load_current_volume ()
{
    ...
    ffatal(current_vol_str != NULL, "no definition of $CURRENT_VOLUME"); 

    errno = 0;
    current_vol = strtol(current_vol_str, NULL, 10);
    ffatal(!errno, "bad value of $CURRENT_VOLUME %s", current_vol_str);
